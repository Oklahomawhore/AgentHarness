...
flowchart LR
  ...
  pkg_dev_workbench["dev-workbench"]
  svc_devWorkbench["ctx.devWorkbench<br/>Profile-configured development task control"]
  pkg_api_remotes["api-remotes"]
  pkg_development_evidence["development-evidence"]
  svc_developmentEvidence["ctx.developmentEvidence<br/>Bounded development evidence retrieval"]
  pkg_development_evidence_reviewed_file["development-evidence-reviewed-file"]
  pkg_mcp["mcp"]
  svc_mcp["ctx.mcp<br/>Host-side MCP call seam"]
  pkg_mcp_client["mcp-client"]
  pkg_agentharness_evidence_gitee_issue["agentharness-evidence-gitee-issue"]
  pkg_mcp_client_setup["mcp-client-setup"]
  svc_mcpClientSetup["ctx.mcpClientSetup<br/>Local MCP client registration"]
  pkg_development_room["development-room"]
  svc_developmentRooms["ctx.developmentRooms<br/>Realtime topic-room membership"]
  ...
  svc_developmentMesh["ctx.developmentMesh<br/>Authenticated development Mesh channels"]
  pkg_development_mesh_websocket["development-mesh-websocket"]
  svc_developmentRoomMesh["ctx.developmentRoomMesh<br/>Room Mesh consumer"]
  svc_developmentTaskMesh["ctx.developmentTaskMesh<br/>Task Mesh consumer"]
  ...
  pkg_dev_workbench --> svc_devWorkbench
  pkg_development_evidence --> svc_developmentEvidence
  pkg_development_evidence_reviewed_file --> svc_developmentEvidence
  pkg_development_mesh --> svc_developmentMesh
  pkg_development_mesh_websocket --> svc_developmentMesh
  pkg_development_room --> svc_developmentRooms
  pkg_development_room_context --> svc_developmentRoomContexts
  pkg_development_room_mesh --> svc_developmentRoomMesh
  ...
  pkg_mcp_client_setup --> svc_mcpClientSetup
  ...
  svc_devWorkbench --> pkg_api_remotes
  svc_developmentMesh --> pkg_development_room_mesh
  svc_developmentMesh --> pkg_development_task_mesh
  svc_developmentRoomContexts --> pkg_api_remotes
  ...
  svc_mcp --> pkg_agentharness_evidence_gitee_issue
  svc_mcpClientSetup --> pkg_api_remotes
...
| `ctx.devWorkbench` | `core` | `dev-workbench` | - | [`api-remotes`](../packages/api/remotes) | - | Owns configured local process trees, bounded output snapshots, and browser-view metadata; deployment profiles provide repository commands and URLs. |
...
| `ctx.developmentRoomMesh` | `core` | [`development-room-mesh`](../packages/collaboration/development-room-mesh) | - | - | - | Replicates hidden Room state and routes membership changes through the generic authenticated Mesh. |
| `ctx.developmentTaskMesh` | `core` | [`development-task-mesh`](../packages/collaboration/development-task-mesh) | - | - | - | Replicates Task, context-block, and assignment deltas and routes mutations to the Task owner node. |
