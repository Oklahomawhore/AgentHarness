...
引导程序校验并安装包含 AgentHarness 与 DeepSeek Harness 的便携运行时，启动服务，打开 `http://127.0.0.1:3080`。它还会配置受支持的本地 MCP 客户端，不会覆盖有冲突的同名配置。npm 包首次发布后，可使用短命令 `npx agentharness`。Unix 需要 curl、tar、sha256sum 或 shasum；Windows x64 需要 PowerShell 和 tar。
