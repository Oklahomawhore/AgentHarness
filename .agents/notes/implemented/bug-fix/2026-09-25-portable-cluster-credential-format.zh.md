...
格式错误的文档、不支持的版本，以及混入顶层凭据的带版本号文档，都会在写入前失败。密钥冲突时需要运维人员选择目标引用。这保留了[公网发行凭据分离](../architecture/2026-08-28-public-portable-release-credential-separation.zh.md)的规则，以及[凭据记录](../architecture/2026-08-13-credential-records-and-authorization-flows.zh.md)定义的布局。
...
便携产物包含 YAML 解析器与原子写入工具。测试结合真实辅助脚本与提供方，覆盖迁移、重启、重复加入和替换。命令预期输出固定本次失败诊断，不含旧输出或浏览器 token。发行浏览器 smoke 连续启动安装产物两次，并验证凭据逐字节不变；只测全新安装无法发现该缺陷。
