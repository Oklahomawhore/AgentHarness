...
The [project README](../../../../README.md) introduces collaborative context engineering through a shared-task example and a screenshot captured from the browser acceptance flow. The example follows publication, individual session connection, and selected inheritance at fixed parent revisions. The README and [user guide](../../../../docs/user/guide/index.md) use the published npm bootstrap, distinguish client configuration from session connection and context acknowledgement, and link cluster setup for separate machines. The README identifies AgentHarness and its maintainer as independent of DeepSeek and links upstream provenance separately from product capabilities.
...
**Lead with a generic workspace description or the long-term vision.** These openings leave readers to infer what participants actually share. A current task example makes publication and inheritance concrete; the white paper owns broader design goals. The README retains the installation command and directs detailed operations to the user guide.
