...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    summary: 'Service Definition for explicit, source-attributed development evidence retrieval.',
    description: 'Service Definition for explicit, source-attributed development evidence retrieval.',
    methods: [
      {
        signature: 'registerProvider(provider: DevelopmentEvidenceProvider): () => void',
        description: 'Register one provider for its plugin lifetime.',
        parameters: [{ name: 'provider', description: 'unique provider implementation.' }],
        returns: 'disposer that unregisters the provider.',
      },
      {
        signature: 'listProviders(): readonly DevelopmentEvidenceProviderRegistration[]',
        description: 'Read the current provider directory in deterministic order.',
        parameters: [],
        returns: 'detached provider registrations sorted by id.',
      },
      {
        signature: 'async query(request: DevelopmentEvidenceQueryRequest, signal?: AbortSignal): Promise<DevelopmentEvidenceQuerySnapshot>',
        description: 'Query every selected provider while preserving empty, denied, and failed outcomes.',
        parameters: [{ name: 'request', description: 'query, optional provider allowlist, and optional per-provider limit.' }, { name: 'signal', description: 'optional caller cancellation.' }],
  ...
  {
    key: 'developmentMesh',
    summary: 'Provider-neutral registry and transport operations for development channels.',
    description: 'Provider-neutral registry and transport operations for development channels.',
    methods: [
      {
        signature: 'register(name: string, channel: DevelopmentMeshChannel): () => void',
        description: 'Register one versioned replication channel.',
        parameters: [{ name: 'name', description: 'stable protocol name such as `development-task/v1`.' }, { name: 'channel', description: 'heads, delta, receive, and optional command handlers.' }],
        returns: 'disposer that removes exactly this registration.',
      },
      {
        signature: 'channel(name: string): DevelopmentMeshChannel | undefined',
        description: 'Read one registered channel for a transport provider.',
        parameters: [{ name: 'name', description: 'exact protocol name.' }],
        returns: 'registered handler or undefined.',
      },
      {
        signature: 'channelNames(): readonly string[]',
        description: 'Return the current registered protocol names.',
        parameters: [],
        returns: 'sorted immutable channel names.',
      },
      {
        signature: 'abstract list(): DevelopmentMeshSnapshot',
        description: 'Read safe connection, cluster, and conflict status.',
        parameters: [],
        returns: 'current provider and peer status without credential material.',
      },
      {
        signature: 'abstract publish(channel: string): void',
        description: 'Notify peers that one channel has newly committed local state.',
        parameters: [{ name: 'channel', description: 'exact registered channel name.' }],
      },
      {
        signature: 'abstract command(ownerNodeId: DevelopmentNodeId, channel: string, payload: unknown): Promise<unknown>',
        description: 'Route one command to its authoritative node.',
        parameters: [{ name: 'ownerNodeId', description: 'authenticated destination node.' }, { name: 'channel', description: 'exact registered channel name.' }, { name: 'payload', description: 'channel-owned wire value.' }],
  ...
  },
  {
    ...
    summary: 'Append-only room context plus request-time selection for live Agent members.',
    description: 'Append-only room context plus request-time selection for live Agent members.',
    methods: [
      {
        signature: '@Remote(\'list\') list(): DevelopmentRoomContextSnapshot',
        description: 'Read the complete append-only context log.',
        parameters: [],
        returns: 'detached entries in append order.',
      },
      {
        signature: 'log(): readonly DevelopmentRoomContextEntry[]',
        description: 'Read the complete append-only context log for persistence and composition.',
        parameters: [],
        returns: 'immutable entries in append order.',
      },
      {
        signature: '@Remote(\'share\') share(request: DevelopmentRoomContextShareRequest): Promise<DevelopmentRoomContextEntry>',
        description: 'Append one current room member\'s explicit text publication.',
        parameters: [{ name: 'request', description: 'room, publisher, and bounded plain text.' }],
        returns: 'committed immutable entry.',
      },
      {
        signature: 'restoreLocalLog(entries: readonly DevelopmentRoomContextEntry[]): void',
        description: 'Restore locally authored entries without writing them again. Existing positions must be byte-equivalent and restore never overwrites divergence.',
        parameters: [{ name: 'entries', description: 'validated durable log whose entries originate on this node.' }],
      },
    ],
  },
  {
    key: 'developmentRoomMesh',
    summary: 'Replicate Room events/presence and route hidden membership changes.',
    description: 'Replicate Room events/presence and route hidden membership changes.',
    methods: [
      {
        signature: 'async join(request: DevelopmentRoomJoinRequest): Promise<void>',
        description: 'Join through the Room creation node.',
        parameters: [{ name: 'request', description: 'hidden Room and participant identity.' }],
      },
      {
        signature: 'async leave(request: DevelopmentRoomLeaveRequest): Promise<void>',
        description: 'Leave through the Room creation node.',
        parameters: [{ name: 'request', description: 'hidden Room and participant identity.' }],
      },
    ],
  },
  {
    ...
    summary: 'Host service for room links, their append-only log, and transient collaborator presence.',
    description: 'Host service for room links, their append-only log, and transient collaborator presence.',
    methods: [
      {
        signature: '@Remote(\'list\') list(): DevelopmentRoomDirectorySnapshot',
        description: 'Read the roster and every discoverable room.',
        parameters: [],
        returns: 'detached projections at the current lease time.',
      },
      {
        signature: 'log(): readonly DevelopmentRoomLogEntry[]',
        description: 'Read the complete append-only room log in local append order.',
        parameters: [],
        returns: 'immutable entries; callers cannot mutate retained state.',
      },
      {
        signature: '@Remote(\'announce\') announce(request: DevelopmentParticipantAnnounceRequest): Promise<DevelopmentRoomDirectorySnapshot>',
        description: 'Announce or update one participant and renew its online lease.',
        parameters: [{ name: 'request', description: 'stable identity, participant kind, and display name.' }],
        returns: 'directory snapshot after the announcement.',
      },
      {
        signature: '@Remote(\'heartbeat\') heartbeat(request: DevelopmentParticipantHeartbeatRequest): Promise<DevelopmentRoomDirectorySnapshot>',
        description: 'Renew one participant lease.',
        parameters: [{ name: 'request', description: 'participant identity whose lease is renewed.' }],
        returns: 'directory snapshot after the lease update.',
      },
      {
        signature: '@Remote(\'withdraw\') withdraw(request: DevelopmentParticipantWithdrawRequest): Promise<DevelopmentRoomDirectorySnapshot>',
        description: 'End one participant\'s online lease.',
        parameters: [{ name: 'request', description: 'participant identity whose retained profile becomes offline.' }],
        returns: 'directory snapshot after the lease ends.',
      },
      {
        signature: '@Remote(\'create\') create(request: DevelopmentRoomCreateRequest): Promise<DevelopmentRoomSnapshot>',
        description: 'Create one discoverable collaboration topic with no implicit member.',
        parameters: [{ name: 'request', description: 'bounded room topic.' }],
        returns: 'room projection after the creation entry commits.',
      },
      {
        signature: 'ensure(request: DevelopmentRoomEnsureRequest): Promise<DevelopmentRoomSnapshot>',
        description: 'Idempotently materialize an internally owned room with a deterministic identity. An existing room must have the same creation node and objective.',
        parameters: [{ name: 'request', description: 'deterministic room identity and bounded internal topic.' }],
        returns: 'the existing or newly committed room projection.',
      },
      {
        signature: '@Remote(\'join\') join(request: DevelopmentRoomJoinRequest): Promise<DevelopmentRoomSnapshot>',
        description: 'Add one announced participant to a room.',
        parameters: [{ name: 'request', description: 'room and participant identities.' }],
        returns: 'room projection after the append, or the current projection when already joined.',
      },
      {
        signature: '@Remote(\'leave\') leave(request: DevelopmentRoomLeaveRequest): Promise<DevelopmentRoomSnapshot>',
        description: 'Leave one room without changing the participant\'s global online lease.',
        parameters: [{ name: 'request', description: 'room and current participant identities.' }],
        returns: 'room projection after the append, or the current projection when already absent.',
      },
      {
        signature: 'restoreLocalLog(entries: readonly DevelopmentRoomLogEntry[]): void',
        description: 'Restore locally authored entries in sequence without writing them again. Existing entries must be byte-equivalent; restore never overwrites divergence.',
        parameters: [{ name: 'entries', description: 'validated durable log whose entries all originate on this node.' }],
      },
      {
        signature: 'acceptPresenceReplica( snapshot: DevelopmentParticipantSnapshot, sourceNodeId: DevelopmentNodeId, ): Promise<DevelopmentParticipantSnapshot>',
        description: 'Accept one peer\'s current participant lease.',
        parameters: [{ name: 'snapshot', description: 'validated participant profile from the peer.' }, { name: 'sourceNodeId', description: 'configured peer identity, which must match the profile node.' }],
        returns: 'current local lease projection after acceptance.',
      },
      {
        signature: 'acceptLogReplica( candidate: DevelopmentRoomLogEntry, sourceNodeId: DevelopmentNodeId, ): Promise<DevelopmentRoomSnapshot>',
        description: 'Append one configured peer\'s room-log entry idempotently.',
        parameters: [{ name: 'candidate', description: 'wire-validated immutable room-link entry.' }, { name: 'sourceNodeId', description: 'configured peer identity, which must originate the entry.' }],
  ...
  },
  {
    ...
    summary: 'Startup barrier published only after the local Room log and persistence listener are ready.',
    description: 'Startup barrier published only after the local Room log and persistence listener are ready.',
    methods: [
      {
        signature: 'readonly nodeId: DevelopmentNodeId',
        description: 'Local node whose append-only Room history has completed recovery.',
        parameters: [],
      },
    ],
  },
  {
    ...
    summary: 'Typed Remote facade that keeps assignment endpoints separate from Task projections.',
    description: 'Typed Remote facade that keeps assignment endpoints separate from Task projections.',
    methods: [
      {
        signature: '@Remote(\'list\') list(): readonly DevelopmentTaskAssignment[]',
        description: 'Read the authoritative Agent-session Task bindings.',
        parameters: [],
        returns: 'all current bindings ordered by participant and binding identity.',
      },
      {
        signature: '@Remote(\'checkout\') checkout(request: DevelopmentTaskCheckoutRequest): Promise<DevelopmentTaskCheckoutResult>',
        description: 'Commit one Agent session\'s Task before best-effort Room reconciliation.',
        parameters: [{ name: 'request', description: 'target Task, local online Agent, and optional binding identity.' }],
        returns: 'assignment, context, and independent Room outcomes.',
      },
      {
        signature: '@Remote(\'clear\') clear(request: DevelopmentTaskClearRequest): Promise<void>',
        description: 'Clear one Agent session\'s Task binding.',
        parameters: [{ name: 'request', description: 'binding and owning Agent participant.' }],
        returns: 'resolution after durable assignment commit.',
      },
      {
        signature: '@Remote(\'acknowledge\') acknowledge(request: DevelopmentTaskAcknowledgeRequest): Promise<DevelopmentTaskAssignment>',
        description: 'Record context delivery for one Agent-session binding.',
        parameters: [{ name: 'request', description: 'binding, Agent, Task, and delivered revision.' }],
  ...
  },
  {
    ...
    summary: 'Host service for immutable Task lineage, shared context, and Agent-session bindings.',
    description: 'Host service for immutable Task lineage, shared context, and Agent-session bindings.',
    methods: [
      {
        signature: '@Remote(\'list\') list(request: DevelopmentTaskListRequest): readonly DevelopmentTaskSnapshot[]',
        description: 'Read bounded current Task projections ordered by recent activity.',
        parameters: [{ name: 'request', description: 'optional participant filter plus result limit.' }],
        returns: 'detached Task projections.',
      },
      {
        signature: '@Remote(\'get\') get(request: DevelopmentTaskGetRequest): DevelopmentTaskSnapshot',
        description: 'Read one current Task projection.',
        parameters: [{ name: 'request', description: 'exact Task identity.' }],
        returns: 'detached current projection.',
      },
      {
        signature: '@Remote(\'lineage\') lineage(request: DevelopmentTaskLineageRequest): DevelopmentTaskLineageSnapshot',
        description: 'Read a bounded ancestor and descendant neighborhood.',
        parameters: [{ name: 'request', description: 'focus Task, depths, and total result limit.' }],
        returns: 'graph Tasks and ids omitted at the requested boundary.',
      },
      {
        signature: 'create(request: DevelopmentTaskCreateRequest): Promise<DevelopmentTaskSnapshot>',
        description: 'Create one Root, Fork, or Merge Task and then materialize its hidden Room.',
        parameters: [{ name: 'request', description: 'immutable parent relation and the new Task\'s context fields.' }],
        returns: 'committed Task; `runtime` is degraded when Room reconciliation fails.',
      },
      {
        signature: '@Remote(\'create\') async createRemote(request: DevelopmentTaskCreateRequest): Promise<DevelopmentTaskCreateResult>',
        description: 'Create one Task through Remote and report hidden-Room materialization separately.',
        parameters: [{ name: 'request', description: 'immutable parent relation and the new Task\'s context fields.' }],
        returns: 'committed Task and runtime state.',
      },
      {
        signature: '@Remote(\'publishContext\') publishContext(request: DevelopmentTaskPublishContextRequest): Promise<DevelopmentTaskSnapshot>',
        description: 'Publish explicit context that may be inherited by later Tasks.',
        parameters: [{ name: 'request', description: 'Task, assigned participant, text, and optional artifact URI.' }],
      ...
      {
        signature: 'checkout(request: DevelopmentTaskCheckoutRequest): Promise<DevelopmentTaskCheckoutResult>',
        description: 'Bind one local online Agent session to a Task, then reconcile Rooms.',
        parameters: [{ name: 'request', description: 'target Task, Agent participant, and optional existing session binding.' }],
      ...
      {
        signature: 'acknowledge(request: DevelopmentTaskAcknowledgeRequest): Promise<DevelopmentTaskAssignment>',
        description: 'Record that one Agent session received a specific Task revision.',
        parameters: [{ name: 'request', description: 'binding, Agent, Task, and delivered revision.' }],
      ...
      },
      {
        ...
        description: 'Read the complete Task event set for storage and Mesh replication.',
        parameters: [],
        returns: 'immutable entries in local acceptance order.',
      },
      {
        signature: 'assignmentLog(): readonly DevelopmentTaskAssignmentLogEntry[]',
        description: 'Read the complete assignment event set for storage and Mesh replication.',
        parameters: [],
        returns: 'immutable entries in local acceptance order.',
      },
      {
        signature: 'blocks(): readonly DevelopmentTaskContextBlock[]',
        description: 'Read all immutable context blocks for storage and Mesh replication.',
        parameters: [],
        returns: 'detached content-addressed blocks.',
      },
      {
        signature: 'restoreContextBlocks(blocks: readonly DevelopmentTaskContextBlock[]): void',
        description: 'Restore context blocks before events that may reference them.',
        parameters: [{ name: 'blocks', description: 'durable blocks whose digest must match their identity.' }],
      },
      {
      ...
      {
        signature: 'restoreAssignmentLog(entries: readonly DevelopmentTaskAssignmentLogEntry[]): void',
        description: 'Restore durable assignment events without rewriting them.',
        parameters: [{ name: 'entries', description: 'validated assignment events.' }],
      },
      {
        signature: 'async acceptContextReplica(block: DevelopmentTaskContextBlock): Promise<void>',
        description: 'Persist and accept one authenticated peer context block idempotently.',
        parameters: [{ name: 'block', description: 'wire-validated content-addressed block.' }],
        returns: 'resolution after durable acceptance.',
      },
      {
        signature: 'acceptLogReplica(candidate: DevelopmentTaskLogEntry, sourceNodeId: DevelopmentNodeId): Promise<DevelopmentTaskSnapshot>',
        description: 'Persist and append one authenticated peer Task event.',
        parameters: [{ name: 'candidate', description: 'wire-validated event.' }, { name: 'sourceNodeId', description: 'authenticated peer identity.' }],
        returns: 'current Task projection.',
      },
      {
        signature: 'acceptAssignmentReplica( candidate: DevelopmentTaskAssignmentLogEntry, sourceNodeId: DevelopmentNodeId, ): Promise<DevelopmentTaskAssignment | undefined>',
        description: 'Persist and append one authenticated peer assignment event.',
        parameters: [{ name: 'candidate', description: 'wire-validated assignment event.' }, { name: 'sourceNodeId', description: 'authenticated participant-home node.' }],
        returns: 'current assignment or undefined after clear.',
      },
      {
        signature: 'async reconcileAllRooms(): Promise<void>',
        description: 'Ensure hidden Rooms exist for every locally owned restored Task.',
        parameters: [],
  ...
  {
    key: 'devWorkbench',
    summary: 'Host runtime for configured development tasks.',
    description: 'Host runtime for configured development tasks.',
    methods: [
      {
        signature: '@Remote(\'list\') list(): DevWorkbenchSnapshot',
        description: 'Read every configured task and its retained output.',
        parameters: [],
        returns: 'Point-in-time task state in configuration order.',
      },
      {
        signature: '@Remote(\'start\') start(id: DevWorkbenchEntryId): Promise<DevWorkbenchEntrySnapshot>',
        description: 'Start one task, or return its current state when it is already live.',
        parameters: [{ name: 'id', description: 'Configured task identity.' }],
        returns: 'State after the serialized start operation.',
      },
      {
        signature: '@Remote(\'stop\') stop(id: DevWorkbenchEntryId): Promise<DevWorkbenchEntrySnapshot>',
        description: 'Stop one task\'s complete process tree and wait until it is gone.',
        parameters: [{ name: 'id', description: 'Configured task identity.' }],
  ...
  {
    key: 'mcp',
    summary: 'Service Definition for transport-independent, Host-side MCP tool calls.',
    description: 'Service Definition for transport-independent, Host-side MCP tool calls.',
    methods: [
      {
        signature: 'registerServer(provider: McpServerProvider): () => void',
        description: 'Register one connected transport provider for its exact live generation.',
        parameters: [{ name: 'provider', description: 'server identity and raw call implementation.' }],
        returns: 'disposer that removes only this registration.',
      },
      {
        signature: 'listServers(): readonly McpServerRegistration[]',
        description: 'List currently callable servers in deterministic order.',
        parameters: [],
        returns: 'detached identities sorted by server id.',
      },
      {
        signature: 'call(request: McpCallRequest, signal: AbortSignal): Promise<McpCallResult>',
        description: 'Call one raw MCP tool without entering the model-facing ToolRuntime pipeline.',
        parameters: [{ name: 'request', description: 'exact server, raw tool name, and JSON arguments.' }, { name: 'signal', description: 'required caller cancellation.' }],
  ...
  {
    key: 'mcpClientSetup',
    summary: 'Trusted Host Remote for detection and explicit browser-initiated setup.',
    description: 'Trusted Host Remote for detection and explicit browser-initiated setup.',
    methods: [
      {
        signature: '@Remote(\'list\') list(): Promise<McpClientSetupSnapshot>',
        description: 'Return the current configuration state of every recognized client.',
        parameters: [],
        returns: 'One point-in-time client detection and configuration snapshot.',
      },
      {
        signature: '@Remote(\'setup\') setup(request: McpClientSetupRequest): Promise<McpClientSetupResult>',
        description: 'Configure exactly one supported client after conflict-safe inspection.',
        parameters: [{ name: 'request', description: 'Client id and optional display-name override.' }],
...
export const EVENT_API: readonly EventApiEntry[] = [
  ...
  {
    name: 'development-evidence/provider-changed',
    mode: 'emit',
    signature: '\'development-evidence/provider-changed\'( provider: DevelopmentEvidenceProviderRegistration, state: \'registered\' | \'unregistered\', ): void',
    summary: 'One evidence provider entered or left the runtime registry.',
    description: 'One evidence provider entered or left the runtime registry.',
    parameters: [{ name: 'provider', description: 'stable provider identity and display label.' }, { name: 'state', description: 'whether the provider is now registered or unregistered.' }],
  },
  {
    name: 'development-mesh/channel-registered',
    mode: 'emit',
    signature: '\'development-mesh/channel-registered\'(name: string): void',
    summary: 'A channel became available for synchronization.',
    description: 'A channel became available for synchronization.',
    parameters: [{ name: 'name', description: 'stable channel protocol name.' }],
  },
  {
    name: 'development-mesh/peer-changed',
    mode: 'emit',
    signature: '\'development-mesh/peer-changed\'(snapshot: DevelopmentMeshPeerSnapshot): void',
    summary: 'A known authenticated peer changed connection state.',
    description: 'A known authenticated peer changed connection state.',
    parameters: [{ name: 'snapshot', description: 'detached state without credentials or signatures.' }],
  },
  {
    name: 'development-room-context/changed',
    mode: 'emit',
    signature: '\'development-room-context/changed\'( entry: DevelopmentRoomContextEntry, origin: DevelopmentRoomContextEventOrigin, ): void',
    summary: 'One shared context entry committed to the append-only log.',
    description: 'One shared context entry committed to the append-only log.',
    parameters: [{ name: 'entry', description: 'exact immutable entry that committed.' }, { name: 'origin', description: 'local publication or durable restore.' }],
  },
  {
    name: 'development-room-context/persist',
    mode: 'parallel',
    signature: '\'development-room-context/persist\'(entry: DevelopmentRoomContextEntry): Promise<void> | void',
    summary: 'Persist one local context entry before it becomes observable.',
    description: 'Persist one local context entry before it becomes observable. A rejecting listener aborts publication.',
    parameters: [{ name: 'entry', description: 'exact next local entry proposed for append.' }],
  },
  {
    name: 'development-room/changed',
    mode: 'emit',
    signature: '\'development-room/changed\'( snapshot: DevelopmentRoomSnapshot, entry: DevelopmentRoomLogEntry, origin: DevelopmentRoomEventOrigin, ): void',
    summary: 'One room-log entry committed and changed its derived room projection.',
    description: 'One room-log entry committed and changed its derived room projection.',
    parameters: [{ name: 'snapshot', description: 'immutable room projection after the append.' }, { name: 'entry', description: 'exact immutable log entry that produced the projection.' }, { name: 'origin', description: 'local append, accepted replica source, or durable restore.' }],
  },
  {
    name: 'development-room/persist',
    mode: 'parallel',
    signature: '\'development-room/persist\'(entry: DevelopmentRoomLogEntry): Promise<void> | void',
    summary: 'Persist one locally authored log entry before it becomes observable.',
    description: 'Persist one locally authored log entry before it becomes observable. A rejecting listener aborts publication; listeners must not mutate the entry.',
    parameters: [{ name: 'entry', description: 'exact next local log entry proposed for append.' }],
  },
  {
    name: 'development-room/presence-changed',
    mode: 'emit',
    signature: '\'development-room/presence-changed\'( snapshot: DevelopmentParticipantSnapshot, origin: DevelopmentRoomEventOrigin, ): void',
    summary: 'One participant renewed or ended its transient online lease.',
    description: 'One participant renewed or ended its transient online lease.',
  ...
  },
  {
    ...
    mode: 'parallel',
    signature: '\'development-task/assignment-persist\'(entry: DevelopmentTaskAssignmentLogEntry): Promise<void> | void',
    summary: 'Persist one Agent session-binding event before it becomes observable.',
    description: 'Persist one Agent session-binding event before it becomes observable.',
    parameters: [{ name: 'entry', description: 'exact next binding event proposed for append.' }],
  },
  {
    name: 'development-task/changed',
    mode: 'emit',
    signature: '\'development-task/changed\'( snapshot: DevelopmentTaskSnapshot, entry: DevelopmentTaskLogEntry, origin: DevelopmentTaskEventOrigin, ): void',
    summary: 'One committed Task context event changed a projection.',
    description: 'One committed Task context event changed a projection.',
    parameters: [{ name: 'snapshot', description: 'immutable Task projection after the append.' }, { name: 'entry', description: 'exact event that produced the projection.' }, { name: 'origin', description: 'local, replica, or restored source.' }],
  },
  {
    name: 'development-task/context-persist',
    mode: 'parallel',
    signature: '\'development-task/context-persist\'(block: DevelopmentTaskContextBlock): Promise<void> | void',
    summary: 'Persist one immutable context block before a Task event may reference it.',
    description: 'Persist one immutable context block before a Task event may reference it.',
    parameters: [{ name: 'block', description: 'content-addressed block proposed for idempotent storage.' }],
  },
  {
    name: 'development-task/persist',
    mode: 'parallel',
    signature: '\'development-task/persist\'(entry: DevelopmentTaskLogEntry): Promise<void> | void',
    summary: 'Persist one Task event before it becomes observable.',
    description: 'Persist one Task event before it becomes observable.',
    parameters: [{ name: 'entry', description: 'exact next event proposed for append.' }],
  },
  {
    name: 'domain/changed',
    mode: 'emit',
  ...
  },
  {
    ...
    mode: 'emit',
    signature: '\'mcp/server-changed\'(server: McpServerRegistration, state: \'registered\' | \'unregistered\'): void',
    summary: 'A callable MCP server was registered or removed.',
    description: 'A callable MCP server was registered or removed.',
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'DevelopmentEvidenceItem',
    declaration: 'export interface DevelopmentEvidenceItem {\n    readonly id: DevelopmentEvidenceItemId;\n    readonly title: string;\n    readonly summary: string;\n    readonly source: string;\n    readonly revision: string;\n    readonly tags?: readonly string[];\n}',
  },
  {
    name: 'DevelopmentEvidenceItemId',
    declaration: 'export type DevelopmentEvidenceItemId = Branded<\'DevelopmentEvidenceItemId\'>;',
  },
  {
    name: 'DevelopmentEvidenceProvider',
    declaration: 'export interface DevelopmentEvidenceProvider {\n    readonly id: DevelopmentEvidenceProviderId;\n    readonly label: string;\n    query(request: DevelopmentEvidenceProviderQuery, signal: AbortSignal): Promise<DevelopmentEvidenceProviderOutcome>;\n}',
  },
  {
    name: 'DevelopmentEvidenceProviderId',
    declaration: 'export type DevelopmentEvidenceProviderId = Branded<\'DevelopmentEvidenceProviderId\'>;',
  },
  {
    name: 'DevelopmentEvidenceProviderOutcome',
    declaration: 'export type DevelopmentEvidenceProviderOutcome = {\n    readonly status: \'available\';\n    readonly items: readonly DevelopmentEvidenceItem[];\n} | {\n    readonly status: \'empty\';\n} | {\n    readonly status: \'denied\';\n    readonly reason: string;\n} | {\n    readonly status: \'failed\';\n    readonly reason: string;\n    readonly retryable: boolean;\n};',
  },
  {
    name: 'DevelopmentEvidenceProviderQuery',
    declaration: 'export interface DevelopmentEvidenceProviderQuery {\n    readonly query: string;\n    readonly limit: number;\n}',
  },
  {
    name: 'DevelopmentEvidenceProviderRegistration',
    declaration: 'export interface DevelopmentEvidenceProviderRegistration {\n    readonly id: DevelopmentEvidenceProviderId;\n    readonly label: string;\n}',
  },
  {
    name: 'DevelopmentEvidenceProviderSnapshot',
    declaration: 'export type DevelopmentEvidenceProviderSnapshot = {\n    readonly providerId: DevelopmentEvidenceProviderId;\n    readonly label: string;\n    readonly status: \'available\';\n    readonly items: readonly DevelopmentEvidenceItem[];\n} | {\n    readonly providerId: DevelopmentEvidenceProviderId;\n    readonly label: string;\n    readonly status: \'empty\';\n} | {\n    readonly providerId: DevelopmentEvidenceProviderId;\n    readonly label: string;\n    readonly status: \'denied\';\n    readonly reason: string;\n} | {\n    readonly providerId: DevelopmentEvidenceProviderId;\n    readonly label: string;\n    readonly status: \'failed\';\n    readonly reason: string;\n    readonly retryable: boolean;\n};',
  },
  {
    name: 'DevelopmentEvidenceQueryRequest',
    declaration: 'export interface DevelopmentEvidenceQueryRequest {\n    readonly query: string;\n    readonly providerIds?: readonly DevelopmentEvidenceProviderId[];\n    readonly limit?: number;\n}',
  },
  {
    name: 'DevelopmentEvidenceQuerySnapshot',
    declaration: 'export interface DevelopmentEvidenceQuerySnapshot {\n    readonly query: string;\n    readonly providers: readonly DevelopmentEvidenceProviderSnapshot[];\n}',
  },
  {
    name: 'DevelopmentMeshChannel',
    declaration: 'export interface DevelopmentMeshChannel {\n    heads(): DevelopmentMeshHeads | Promise<DevelopmentMeshHeads>;\n    read(after: DevelopmentMeshHeads): readonly unknown[] | Promise<readonly unknown[]>;\n    receive(events: readonly unknown[], sourceNodeId: DevelopmentNodeId): void | Promise<void>;\n    command?(payload: unknown, sourceNodeId: DevelopmentNodeId): unknown;\n    peerOffline?(nodeId: DevelopmentNodeId): void | Promise<void>;\n}',
  },
  {
    name: 'DevelopmentMeshHeads',
    declaration: 'export type DevelopmentMeshHeads = Readonly<Record<string, number>>;',
  },
  {
    name: 'DevelopmentMeshPeerSnapshot',
    declaration: 'export interface DevelopmentMeshPeerSnapshot {\n    readonly nodeId: DevelopmentNodeId;\n    readonly direction: \'dial\' | \'accept\';\n    readonly state: \'connecting\' | \'online\' | \'offline\' | \'isolated\';\n    readonly lastSeenAt?: number;\n    readonly lastError?: string;\n    readonly pendingSync: number;\n    readonly conflicts: number;\n}',
  },
  {
    name: 'DevelopmentMeshSnapshot',
    declaration: 'export interface DevelopmentMeshSnapshot {\n    readonly nodeId: DevelopmentNodeId;\n    readonly clusterId: string;\n    readonly secretFingerprint: string;\n    readonly peers: readonly DevelopmentMeshPeerSnapshot[];\n}',
  },
  {
    name: 'DevelopmentNodeId',
    declaration: 'export type DevelopmentNodeId = Branded<\'DevelopmentNodeId\'>;',
  },
  {
    name: 'DevelopmentParticipantAnnounceRequest',
    declaration: 'export interface DevelopmentParticipantAnnounceRequest {\n    readonly id: DevelopmentParticipantId;\n    readonly kind: DevelopmentParticipantKind;\n    readonly displayName: string;\n}',
  },
  {
    name: 'DevelopmentParticipantHeartbeatRequest',
    declaration: 'export interface DevelopmentParticipantHeartbeatRequest {\n    readonly participantId: DevelopmentParticipantId;\n}',
  },
  {
    name: 'DevelopmentParticipantId',
    declaration: 'export type DevelopmentParticipantId = Branded<\'DevelopmentParticipantId\'>;',
  },
  {
    name: 'DevelopmentParticipantKind',
    declaration: 'export type DevelopmentParticipantKind = \'human\' | \'agent\';',
  },
  {
    name: 'DevelopmentParticipantSnapshot',
    declaration: 'export interface DevelopmentParticipantSnapshot {\n    readonly id: DevelopmentParticipantId;\n    readonly nodeId: DevelopmentNodeId;\n    readonly kind: DevelopmentParticipantKind;\n    readonly displayName: string;\n    readonly presence: \'online\' | \'offline\';\n    readonly lastSeenAt: number;\n}',
  },
  {
    name: 'DevelopmentParticipantWithdrawRequest',
    declaration: 'export interface DevelopmentParticipantWithdrawRequest {\n    readonly participantId: DevelopmentParticipantId;\n}',
  },
  {
    name: 'DevelopmentRoomContextEntry',
    declaration: 'export interface DevelopmentRoomContextEntry {\n    readonly nodeId: DevelopmentNodeId;\n    readonly seq: number;\n    readonly at: number;\n    readonly roomId: DevelopmentRoomId;\n    readonly participantId: DevelopmentParticipantId;\n    readonly text: string;\n}',
  },
  {
    name: 'DevelopmentRoomContextEventOrigin',
    declaration: 'export type DevelopmentRoomContextEventOrigin = {\n    readonly kind: \'local\';\n    readonly nodeId: DevelopmentNodeId;\n} | {\n    readonly kind: \'restored\';\n    readonly nodeId: DevelopmentNodeId;\n};',
  },
  {
    name: 'DevelopmentRoomContextShareRequest',
    declaration: 'export interface DevelopmentRoomContextShareRequest {\n    readonly roomId: DevelopmentRoomId;\n    readonly participantId: DevelopmentParticipantId;\n    readonly text: string;\n}',
  },
  {
    name: 'DevelopmentRoomContextSnapshot',
    declaration: 'export interface DevelopmentRoomContextSnapshot {\n    readonly nodeId: DevelopmentNodeId;\n    readonly entries: readonly DevelopmentRoomContextEntry[];\n}',
  },
  {
    name: 'DevelopmentRoomCreateRequest',
    declaration: 'export interface DevelopmentRoomCreateRequest {\n    readonly objective: string;\n}',
  },
  {
    name: 'DevelopmentRoomDirectorySnapshot',
    declaration: 'export interface DevelopmentRoomDirectorySnapshot {\n    readonly nodeId: DevelopmentNodeId;\n    readonly presenceTtlMs: number;\n    readonly participants: readonly DevelopmentParticipantSnapshot[];\n    readonly rooms: readonly DevelopmentRoomSnapshot[];\n}',
  },
  {
    name: 'DevelopmentRoomEnsureRequest',
    declaration: 'export interface DevelopmentRoomEnsureRequest extends DevelopmentRoomCreateRequest {\n    readonly roomId: DevelopmentRoomId;\n}',
  },
  {
    name: 'DevelopmentRoomEventOrigin',
    declaration: 'export type DevelopmentRoomEventOrigin = {\n    readonly kind: \'local\';\n    readonly nodeId: DevelopmentNodeId;\n} | {\n    readonly kind: \'replica\';\n    readonly nodeId: DevelopmentNodeId;\n} | {\n    readonly kind: \'restored\';\n    readonly nodeId: DevelopmentNodeId;\n};',
  },
  {
    name: 'DevelopmentRoomId',
    declaration: 'export type DevelopmentRoomId = Branded<\'DevelopmentRoomId\'>;',
  },
  {
    name: 'DevelopmentRoomJoinRequest',
    declaration: 'export interface DevelopmentRoomJoinRequest {\n    readonly roomId: DevelopmentRoomId;\n    readonly participantId: DevelopmentParticipantId;\n}',
  },
  {
    name: 'DevelopmentRoomLeaveRequest',
    declaration: 'export interface DevelopmentRoomLeaveRequest {\n    readonly roomId: DevelopmentRoomId;\n    readonly participantId: DevelopmentParticipantId;\n}',
  },
  {
    name: 'DevelopmentRoomLogChange',
    declaration: 'export type DevelopmentRoomLogChange = {\n    readonly kind: \'room-created\';\n    readonly objective: string;\n} | {\n    readonly kind: \'participant-joined\';\n    readonly participantId: DevelopmentParticipantId;\n} | {\n    readonly kind: \'participant-left\';\n    readonly participantId: DevelopmentParticipantId;\n};',
  },
  {
    name: 'DevelopmentRoomLogEntry',
    declaration: 'export interface DevelopmentRoomLogEntry {\n    readonly nodeId: DevelopmentNodeId;\n    readonly seq: number;\n    readonly at: number;\n    readonly roomId: DevelopmentRoomId;\n    readonly change: DevelopmentRoomLogChange;\n}',
  },
  {
    name: 'DevelopmentRoomSnapshot',
    declaration: 'export interface DevelopmentRoomSnapshot {\n    readonly id: DevelopmentRoomId;\n    readonly creationNodeId: DevelopmentNodeId;\n    readonly objective: string;\n    readonly createdAt: number;\n    readonly updatedAt: number;\n    readonly participantIds: readonly DevelopmentParticipantId[];\n    readonly participants: readonly DevelopmentParticipantSnapshot[];\n}',
  },
  {
  ...
  },
  {
    ...
    declaration: 'export interface DevelopmentTaskAssignment {\n    readonly bindingId: DevelopmentTaskBindingId;\n    readonly participantId: DevelopmentParticipantId;\n    readonly taskId: DevelopmentTaskId;\n    readonly sessionLabel?: string;\n    readonly assignedAt: number;\n    readonly acknowledgedRevision?: number;\n}',
  },
  {
    name: 'DevelopmentTaskAssignmentLogEntry',
  ...
  },
  {
    ...
    declaration: 'export type DevelopmentTaskEventOrigin = {\n    readonly kind: \'local\';\n    readonly nodeId: DevelopmentNodeId;\n} | {\n    readonly kind: \'replica\';\n    readonly nodeId: DevelopmentNodeId;\n} | {\n    readonly kind: \'restored\';\n    readonly nodeId: DevelopmentNodeId;\n};',
  },
  {
    name: 'DevelopmentTaskGetRequest',
    declaration: 'export interface DevelopmentTaskGetRequest {\n    readonly taskId: DevelopmentTaskId;\n}',
  },
  {
    name: 'DevelopmentTaskId',
    declaration: 'export type DevelopmentTaskId = Branded<\'DevelopmentTaskId\'>;',
  },
  {
    name: 'DevelopmentTaskInheritedSource',
    declaration: 'export interface DevelopmentTaskInheritedSource {\n    readonly parent: DevelopmentTaskParentRef;\n    readonly objective: string;\n    readonly scope: string;\n    readonly context: readonly DevelopmentTaskContextPublication[];\n}',
  },
  {
    name: 'DevelopmentTaskLineageRequest',
    declaration: 'export interface DevelopmentTaskLineageRequest {\n    readonly taskId: DevelopmentTaskId;\n    readonly ancestorDepth?: number;\n    readonly descendantDepth?: number;\n    readonly limit?: number;\n}',
  },
  {
    name: 'DevelopmentTaskLineageSnapshot',
    declaration: 'export interface DevelopmentTaskLineageSnapshot {\n    readonly tasks: readonly DevelopmentTaskSnapshot[];\n    readonly boundaryTaskIds: readonly DevelopmentTaskId[];\n}',
  },
  {
    name: 'DevelopmentTaskListRequest',
  ...
  },
  {
    ...
    declaration: 'export type DevWorkbenchEntryId = Branded<\'DevWorkbenchEntryId\'>;',
  },
  {
    name: 'DevWorkbenchEntryPhase',
    declaration: 'export type DevWorkbenchEntryPhase = \'idle\' | \'running\' | \'stopping\' | \'stopped\' | \'exited\' | \'failed\';',
  },
  {
    name: 'DevWorkbenchEntrySnapshot',
    declaration: 'export interface DevWorkbenchEntrySnapshot {\n    readonly id: DevWorkbenchEntryId;\n    readonly label: string;\n    readonly argv: readonly string[];\n    readonly cwd: string;\n    readonly views: readonly DevWorkbenchView[];\n    readonly phase: DevWorkbenchEntryPhase;\n    readonly readiness?: DevWorkbenchReadinessSnapshot;\n    readonly pid?: number;\n    readonly startedAt?: number;\n    readonly finishedAt?: number;\n    readonly outcome?: DevWorkbenchOutcome;\n    readonly error?: string;\n    readonly stdout: string;\n    readonly stderr: string;\n    readonly stdoutLossy: boolean;\n    readonly stderrLossy: boolean;\n    readonly stdoutSpillPath?: string;\n    readonly stderrSpillPath?: string;\n}',
  },
  {
    name: 'DevWorkbenchOutcome',
    declaration: 'export interface DevWorkbenchOutcome {\n    readonly exitCode: number | null;\n    readonly signal: string | null;\n}',
  },
  {
    name: 'DevWorkbenchReadinessSnapshot',
    declaration: 'export interface DevWorkbenchReadinessSnapshot {\n    readonly url: string;\n    readonly state: DevWorkbenchReadinessState;\n    readonly checkedAt?: number;\n    readonly statusCode?: number;\n    readonly error?: string;\n}',
  },
  {
    name: 'DevWorkbenchReadinessState',
    declaration: 'export type DevWorkbenchReadinessState = \'checking\' | \'ready\' | \'delayed\';',
  },
  {
    name: 'DevWorkbenchSnapshot',
    declaration: 'export interface DevWorkbenchSnapshot {\n    readonly entries: readonly DevWorkbenchEntrySnapshot[];\n}',
  },
  {
    name: 'DevWorkbenchView',
    declaration: 'export interface DevWorkbenchView {\n    readonly id: DevWorkbenchViewId;\n    readonly label: string;\n    readonly url: string;\n}',
  },
  {
    name: 'DevWorkbenchViewId',
    declaration: 'export type DevWorkbenchViewId = Branded<\'DevWorkbenchViewId\'>;',
  },
  {
    name: 'DiffCallView',
  ...
  {
    name: 'McpCallRequest',
    declaration: 'export interface McpCallRequest {\n    readonly serverId: McpServerId;\n    readonly toolName: string;\n    readonly arguments: Readonly<Record<string, JsonValue>>;\n}',
  },
  {
    name: 'McpCallResult',
    declaration: 'export interface McpCallResult {\n    readonly content: readonly JsonValue[];\n    readonly structuredContent?: JsonValue;\n}',
  },
  {
    name: 'McpClientId',
    declaration: 'export type McpClientId = \'cursor\' | \'codex\' | \'claude-code\' | \'workbuddy\' | \'codebuddy\' | \'trae\' | \'doubao\';',
  },
  {
    name: 'McpClientSetupMechanism',
    declaration: 'export type McpClientSetupMechanism = \'json\' | \'cli\' | \'manual\';',
  },
  {
  ...
  {
    name: 'McpServerProvider',
    declaration: 'export interface McpServerProvider {\n    readonly id: McpServerId;\n    call(request: Omit<McpCallRequest, \'serverId\'>, signal: AbortSignal): Promise<McpCallResult>;\n}',
  },
  {
    name: 'McpServerRegistration',
    declaration: 'export interface McpServerRegistration {\n    readonly id: McpServerId;\n}',
  },
  {
    name: 'Message',
