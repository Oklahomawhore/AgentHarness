...
import McpService, { type McpServerId } from '@deepseek-ai/dsh-mcp'
...
async function mountRegistry(): Promise<Context> {
  ...
  await ctx.plugin(McpService, { maxServers: 8 })
  return ctx
}
...
describe('mcp-client plugin module exports', () => {
  it('exports name, inject, and Config', () => {
    ...
    expect(inject).toEqual(['mcp', 'tools'])
...
describe('apply (plugin lifecycle)', () => {
  ...
  it('publishes a lossless direct Host-call route for the connected generation', async () => {
    await apply(ctx, stdioConfig)
    const registry = ctx.mcp

    await expect(ctx.mcp.call({
      serverId: 'srv' as McpServerId,
      toolName: 'remote',
      arguments: { issue_id: 'IK9O1Y' },
    }, new AbortController().signal)).resolves.toEqual({
      content: [{ type: 'text', text: 'ok' }],
    })
    expect(mockCallTool).toHaveBeenCalledWith(
      { name: 'remote', arguments: { issue_id: 'IK9O1Y' } },
      undefined,
      expect.objectContaining({ timeout: 60_000 }),
    )

    expect(mockCallTool.mock.calls[0]?.[2]).toHaveProperty('signal', expect.any(AbortSignal))
  ...
  it('releases the serverName reservation on dispose', async () => {
    ...
    await first.plugin(McpService, { maxServers: 8 })
    ...
    await second.plugin(McpService, { maxServers: 8 })
