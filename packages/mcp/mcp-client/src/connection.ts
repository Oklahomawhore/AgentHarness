...
import type { McpServerId } from '@deepseek-ai/dsh-mcp'
import { MAX_TIMER_DELAY_MS } from '@deepseek-ai/dsh-timeout'
import { createTransport } from './transport.ts'
import { callMcpTool, syncTools } from './tools.ts'
...
export function startConnection(ctx: Context, config: Config, policy: ResolvedReconnectPolicy): ConnectionHandle {
  ...
  /** Direct Host-call registration for the current connected generation. */
  ...
  /** Remove the direct Host-call route before its client generation closes. */
  function unregisterServer(): void {
    disposeServer?.()
    disposeServer = undefined
  }

  /** Publish the exact connected generation for trusted Host Consumers. */
  function registerServer(generation: Client): void {
    disposeServer = ctx.mcp.registerServer({
      id: config.serverName as McpServerId,
      call: (request, signal) => callMcpTool(
        generation,
        request.toolName,
        request.arguments,
        signal,
  ...
  async function connectGeneration(startup: boolean): Promise<void> {
    ...
    try {
      ...
      if (isCurrent(generation)) registerServer(generation)
    } catch (error) {
