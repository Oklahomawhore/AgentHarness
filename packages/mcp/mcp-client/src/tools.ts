...
import type { McpCallResult } from '@deepseek-ai/dsh-mcp'
...
/**
 * Invoke one raw MCP tool for a trusted Host consumer without changing its protocol content.
 * @param client - connected MCP client generation.
 * @param rawName - server-owned tools/call name.
 * @param args - JSON arguments.
 * @param signal - caller cancellation.
 * @param timeoutMs - request timeout in milliseconds.
 * @returns canonical Host-side MCP result.
 */
export async function callMcpTool(
  ...
  const result = await callToolUncached(client, rawName, args, signal, timeoutMs)
