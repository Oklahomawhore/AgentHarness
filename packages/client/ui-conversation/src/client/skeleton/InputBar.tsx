        ...
        <div ref={scrollRef} className={css.scroll} data-input-scroll>
          <div className={css.grow}>
            <ComposerContentEditable
              ...
              onClick={() => {
                if (blocked?.onActivate !== undefined) blocked.onActivate()
                else if (!workspaceTrigger && window.location.hash === '#agentharness=collaboration') {
                  window.dispatchEvent(new Event('dsh:configure-model'))
                }
              }}
              aria-haspopup={workspaceTrigger ? 'menu' : undefined}
              aria-expanded={workspaceTrigger ? workspacePickerOpen : undefined}
              tabIndex={workspaceTrigger || blocked?.onActivate !== undefined ? 0 : undefined}
              onKeyDown={blocked?.onActivate === undefined ? (workspaceTrigger ? onWorkspaceKeyDown : undefined) : (event) => {
