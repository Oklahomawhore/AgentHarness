...
`dsh-client-ui-settings-models` 配置提供方 API 密钥、模型列表和自定义 pi-ai 路由。它把提供方目录、设置和凭据描述合并到一份快照。在 AgentHarness 启动器中，点击对话输入区且没有可用提供方时才显示 DeepSeek 密钥弹窗；仅打开 AgentHarness 不会索取密钥。其他 Web 启动方式保留有序引导。
...
用户点击受阻的对话输入区后，DeepSeek 步骤从同一份合并快照投影就绪状态。用户已经能够到达的**任何**提供方都会直接结束该步骤、不做渲染；只有没有任何提供方的用户才会被询问官方 DeepSeek 密钥。「稍后配置」只完成这次协调器遍历；适配器缺失、路由不活动、合并失败、只读部署或能力不可用时，该步骤不渲染即完成——Models 仍是诊断界面。
