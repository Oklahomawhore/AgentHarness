 ...
 * the onboarding coordinator mounts the credential step when an AgentHarness user activates
 * a blocked composer; other Web launches retain their ordered steps. Visible dialog chrome belongs
...
export function SettingsRoot(props: SettingsRootComponentProps) {
  ...
  const [configureRequested, setConfigureRequested] = useState(false)
  ...
  useEffect(() => {
    const request = (): void => { setConfigureRequested(true) }
    window.addEventListener('dsh:configure-model', request)
    return () => { window.removeEventListener('dsh:configure-model', request) }
  }, [])
  ...
  const agentHarnessLaunch = window.location.hash === '#agentharness=collaboration'
  const onboardingStep = configureRequested
    ? onboardingSteps.find(step => step.id === 'deepseek-official')
    : !agentHarnessLaunch && onboardingActive
