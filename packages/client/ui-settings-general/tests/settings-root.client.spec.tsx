...
describe('SettingsPanel navigation', () => {
  ...
  it('defers credential setup on an AgentHarness launch until conversation activation', () => {
    const previous = window.location.href
    window.history.replaceState(null, '', '/#agentharness=collaboration')
    try {
      const { renderSlot } = mount({ steps: [{ id: 'deepseek-official', order: 0 }] })
      expect(renderSlot.mock.calls.some(call => call[0] === 'settings.onboarding')).toBe(false)
      act(() => { window.dispatchEvent(new Event('dsh:configure-model')) })
      const step = renderSlot.mock.calls.find(call => call[0] === 'settings.onboarding')
      expect(step?.[1]).toMatchObject({ stepId: 'deepseek-official' })
      act(() => { (step?.[1] as { complete: () => void }).complete() })
      const count = renderSlot.mock.calls.filter(call => call[0] === 'settings.onboarding').length
      expect(count).toBe(1)
    } finally {
