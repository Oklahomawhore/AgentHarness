...
- **Branch workflow.** `main` is the only long-lived development branch. Base topic branches and ordinary PRs on current `origin/main`; dependent PRs may target their parent. Releases tag verified `main` commits. Never force-push `main` or move published tags ([collaboration rules](docs/development.md#agentharness-branch-workflow)).
