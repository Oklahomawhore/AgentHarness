...
/** Internal AgentHarness distribution endpoint approved to come from the tracked launch environment. */
const AGENTHARNESS_PROVIDER_BASE_URL = /^\s*baseURL\s*:\s*!!js\s+process\.env\.AGENTHARNESS_PROVIDER_BASE_URL\s*$/
  ...
  for (const glob of SHIPPED_CONFIG_GLOBS) {
    for (const file of globSync(glob, { cwd: root })) {
      ...
      readFileSync(resolve(root, rel), 'utf8').split('\n').forEach((line, index) => {
        ...
        if (rel === 'packages/bundle/base/cordis.patch.yml' && AGENTHARNESS_PROVIDER_BASE_URL.test(line)) return
        failures.push(
