...
describe('docsPages locale routes', () => {
  it('renders both locale roots as localized landing pages', () => {
    ...
    for (const page of homes) {
      ...
      expect(projected).toContain(`<LandingPage locale="${page.locale === 'root' ? 'zh' : 'en'}" />`)
      expect(projected).toContain('title: AgentHarness')
...
describe('projectedPageContent', () => {
  ...
  it('keeps the landing-page body for locale home pages', () => {
...
describe('raw Markdown projection of the published manifest', () => {
  ...
  it('emits home pages with their bodies instead of the frontmatter stub', () => {
    for (const route of ['index.md', 'en/index.md']) {
      ...
      expect(home, route).toContain(`<LandingPage locale="${route === 'index.md' ? 'zh' : 'en'}" />`)
    }
  })
