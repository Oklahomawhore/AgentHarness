...
export function languageSwitcherLinkOffset(
  ...
  if (headingIndex < 0 && frontmatterEnd < 5) return undefined
  for (const node of tree.children.slice(headingIndex < 0 ? 0 : headingIndex + 1)) {
    if (headingIndex < 0 && (node.position?.end.offset ?? 0) <= frontmatterEnd) continue
